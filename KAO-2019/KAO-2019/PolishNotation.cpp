...
namespace Polish
{
	bool isPolish(int i, LexAnalysis::Table& lex)
	{
		...
		int countLex = 0, countParm = 1, posLex = i;
		char* buf = new char[1];

		bool findComma = false;

		for (i; lex.lextable.table[i].lexema != LEX_POINT; i++, countLex++)
		{
	...
	bool GoPolish(LexAnalysis::Table& lex)
	{
		...
		for (int i = 0; i < lex.lextable.size; i++)
		{
			if (lex.lextable.table[i].lexema == LEX_EQUAL || (lex.lextable.table[i].lexema == LEX_ID && lex.lextable.table[i - 1].lexema != LEX_EQUAL &&
				lex.idtable.table[lex.lextable.table[i].idxTI].idtype == IT::F  && lex.lextable.table[i - 2].lexema != LEX_FUNCTION))
			{
				rc = isPolish(i + 1, lex);
				if (!rc)	return rc;
			}
